export default function ElSalvadorRecapPage() {
  return (
    <div className="mx-auto max-w-3xl px-6 py-24 text-center">
      ...
      {/* TODO: photo gallery — pull the real gallery images from the current
          WordPress media library once we're cleared to migrate them. */}
      <div className="mt-16 grid grid-cols-2 gap-4 sm:grid-cols-3">
        {Array.from({ length: 9 }).map((_, i) => (
          <div
            key={i}
            className="aspect-square bg-gradient-to-br from-sea to-sand/40"
          />
        ))}
      </div>
    </div>
